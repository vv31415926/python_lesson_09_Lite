...
class GoGame:
    def __init__(self):
        ...
        self.card_in = ''
        self.card_answer = ''
        self.k = Koloda()  # получить полную колоду карт
    ...
    def win_gamer(self, ig, num=False):
        ...
        if self.slave_gamer == ig:   # к кому ходят
            print(' '*10 + 'Стол:'+self.win_table()+'   колода:'+str(self.k.get_count() )  )
        else:
            print()
    ...
    def assept_cards(self):
        for i in range(self.gamers[self.main_gamer].get_count(), 6):
            if self.k.get_count() > 0:
                self.gamers[self.main_gamer].assept_cards(self.k.card_out())
        for i in range(self.gamers[self.slave_gamer].get_count(), 6):
            if self.k.get_count() > 0:
                self.gamers[self.slave_gamer].assept_cards(self.k.card_out())
    ...
    def start(self):
        ...
        for i in range(6):
            for i in range( len(self.gamers) ):
                self.gamers[i].assept_cards(  self.k.card_out()  )
